...
class rectification():
    ...
    def __init__(self, image_name):
        """r_image: ndarray
            Path of Image which has  to be rectified."""
        #Forse da cambiare in base alla pipeline, passandogli già l'img come np.array?
        self.r_image = image_name
    ...
    def ransac_3_line(self, edgelets, focal_length, num_ransac_iter=2000,
                      threshold_inlier=5):
        ...
        for ransac_iter in range(num_ransac_iter):
            ...
            vp1_votes = self.compute_votes(edgelets, vp1, threshold_inlier)
            vp2_votes = self.compute_votes(edgelets, vp2, threshold_inlier)
    ...
    def compute_homography_and_warp(self, image, vp1, vp2, clip=True, clip_factor=3):
    ...
    def rectify_image(self, clip_factor=6, algorithm='independent',
                      reestimate=False):
        ...
        if algorithm == 'independent':
            ...
            vp1 = self.ransac_vanishing_point(edgelets1, 2000, threshold_inlier=5)
            if reestimate:
                vp1 = self.reestimate_model(vp1, edgelets1, 5)

            # Remove inlier to remove dominating direction.
            edgelets2 = self.remove_inliers(vp1, edgelets1, 10)

            # Find second vanishing point
            vp2 = self.ransac_vanishing_point(edgelets2, 2000, threshold_inlier=5)
            if reestimate:
                vp2 = self.reestimate_model(vp2, edgelets2, 5)
        elif algorithm == '3-line':
            focal_length = None
            vp1, vp2 = self.ransac_3_line(edgelets1, focal_length,
        ...
        warped_img = self.compute_homography_and_warp(image, vp1, vp2,
                                                 clip_factor=clip_factor)

        #metodi per vedere vanishing point e spigoli utilizzati
        #(nella pipeline possiamo toglierle queste visualizzazioni, aggiunte solo magari per mettere
        #risultati nel paper)
        self.vis_model(image, vp2)
        self.vis_edgelets(image, edgelets2)
